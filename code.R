# ==========================================================
# Comprehensive Analysis Pipeline (UKB-MDD)
# ==========================================================
library(data.table)
library(parallel)
library(caret)
...
# ----------------------------------------------------------
feature_data <- readRDS('/home/share/zsj/szj/protein_feature0.5_knn.rds')

# Identify selected and random samples
selected_samples_df <- read.csv('/home/share/zsj/proteomics/proteomic_selected_sample.csv')
selected_sample_ids <- as.character(selected_samples_df$eid[which(selected_samples_df[, 3] == 1)])
random_sample_ids <- setdiff(base::rownames(feature_data), selected_sample_ids)

test_features <- feature_data[selected_sample_ids, ]
train_features <- feature_data[random_sample_ids, ]
combined_features <- rbind(train_features, test_features)

# Load diagnosis data
mdd_ids <- readRDS('/home/share/zsj/snptrans/MDD_MP.rds')
dsmb_data <- readRDS('/home/share/zsj/snptrans/MDD_DSMB.rds')
control_list <- readRDS('/home/share/zsj/snptrans/CTL.rds')

# Calculate DSMB scores and identify targets
positive_diagnosis_counts <- unlist(mclapply(seq_len(nrow(dsmb_data)), function(i) length(which(dsmb_data[i, ] == 1)), mc.cores = 5))
target_diagnosis_indices <- which(positive_diagnosis_counts == 9)

dsmb_cleaned <- as.data.frame(na.omit(dsmb_data - 1))
dsmb_scores <- rowSums(dsmb_cleaned)
dsmb_scores <- dsmb_scores[dsmb_scores > 0]
score_df <- data.frame(id = names(dsmb_scores), score = dsmb_scores)

# Process control samples
control_list$dsmb <- target_diagnosis_indices
intersected_controls <- Reduce(intersect, control_list)
control_ids <- base::rownames(dsmb_data)[intersected_controls]
control_ids <- setdiff(control_ids, mdd_ids)

# Construct and align training labels
labels_df_train <- data.frame(
  id = c(mdd_ids, control_ids),
  label = c(rep(1, length(mdd_ids)), rep(0, length(control_ids)))
)
rownames(labels_df_train) <- labels_df_train$id

aligned_train_ids <- intersect(base::rownames(train_features), labels_df_train$id)
labels_df_train <- labels_df_train[aligned_train_ids, ]
train_features <- train_features[aligned_train_ids, ]
train_data_final <- cbind(label = labels_df_train$label, train_features)

# Construct and align testing labels
labels_df_test <- data.frame(
  id = c(mdd_ids, control_ids),
  label = c(rep(1, length(mdd_ids)), rep(0, length(control_ids)))
)
base::rownames(labels_df_test) <- labels_df_test$id

aligned_test_ids <- intersect(base::rownames(test_features), labels_df_test$id)
labels_df_test <- labels_df_test[aligned_test_ids, ]
test_features <- test_features[aligned_test_ids, ]
test_data_final <- cbind(label = labels_df_test$label, test_features)

# ----------------------------------------------------------
# 2. Load Confounders
# ----------------------------------------------------------
confounder_metadata <- data.table::fread('/home/share/zsj/szj/ukb_info_5w_cell_median.csv', data.table = FALSE)
rownames(confounder_metadata) <- confounder_metadata$IID
confounder_metadata <- confounder_metadata[, -1] # Remove ID column

# Format metadata variables
confounder_metadata$sample_age <- confounder_metadata$sample_age / 365
confounder_metadata$Season <- as.factor(confounder_metadata$Season)
confounder_metadata$ethnic <- as.factor(confounder_metadata$ethnic)

saveRDS(confounder_metadata, '/home/share/zsj/ukb_pro_mdd/ukb_pro_confounders.rds')
confounder_metadata <- readRDS('/home/share/zsj/ukb_pro_mdd/ukb_pro_confounders.rds')

# ----------------------------------------------------------
# 3. Base Modeling Functions
# ----------------------------------------------------------
...
#' Train a model utilizing cross-validation with error handling
...
#' Train a simple classification model with tuning
...
#' Perform Step 2 feature selection based on specified methods
step2_feature_selection <- function(selection_method = NULL, feature_data = NULL, target_labels = NULL, target_cutoff = NULL, prediction_mode = NULL, classifier_name = NULL) {
  if (prediction_mode %in% c('probability', 'classification')) {
    ...
    # Placeholder for future regression logic
  }
}

#' Add unmapped features to the dataset
...
# ----------------------------------------------------------
# 5. BioM2 Cross-Validation & Parameter Tuning
# ----------------------------------------------------------
print(table(train_data_final$label))

# Load database and annotations
pathway_db <- readRDS('/home/zhangshunjie/BioMM/3.21_org.Hs.egGO2EG_BP.rds')
feature_annotations <- readRDS('/home/share/zsj/proteomics/protein_anno.rds')
...
saveRDS(final_compiled_results, '/home/share/zsj/ukb_pro_mdd/ukb_pro_lifetime_mdd_biom2_5cv_100incv_para_20250804.rds')

# ----------------------------------------------------------
# 6. Optimal Parameter Validation (Cross-Validation & Independent)
# ----------------------------------------------------------
cv_results_df <- readRDS('/home/share/zsj/ukb_pro_mdd/ukb_pro_lifetime_mdd_biom2_5cv_100incv_para_20250804.rds')
...
# 6.1 Run BioM2 with Optimal Parameters (Cross-Validation on Train)
set.seed(666)
...
saveRDS(biom2_train_results, '/home/share/zsj/ukb_pro_mdd/train_predict_train_20250805.rds')

# 6.2 Run BioM2 with Optimal Parameters (Independent Validation on Test)
...
saveRDS(biom2_test_results, '/home/share/zsj/ukb_pro_mdd/train_predict_test_20250805.rds')

# ----------------------------------------------------------
# 7. Lasso Regression for Important Pathway Selection
# ----------------------------------------------------------
library(BioM2)
library(GO.db)
...
saveRDS(lasso_top_pathways, '/home/share/zsj/ukb_pro_mdd/ukb_pro_lifetime_mdd_train_lasso_top_path.rds')
...
saveRDS(lasso_final_weights, '/home/share/zsj/ukb_pro_mdd/lasso_92path_20250805.rds')

# ----------------------------------------------------------
# 8. Lasso Regression for Important Protein Selection
# ----------------------------------------------------------
# Utilizing the selected top pathways to extract specific proteins
selected_pathways_df <- readRDS('/home/share/zsj/ukb_pro_mdd/lasso_92path_20250805.rds')
...
# Extract features based on correlation/wilcox
...
# ----------------------------------------------------------
# 9. Phenotype Association Analysis
# ----------------------------------------------------------
library(MASS)
library(dplyr)

phenotype_data <- readRDS('/home/share/zsj/phenotype.rds')
biom2_validation_data <- readRDS('/home/share/zsj/ukb_pro_mdd/train_predict_test_20250805.rds')
...
# ----------------------------------------------------------
# 10. Mendelian Randomization (MR): Protein -> MDD
# ----------------------------------------------------------
library(TwoSampleMR)
library(ggplot2)
library(data.table)
...
mdd_gwas_data <- fread('/home/share/zsj/mdd_gwas/mdd2023/mdd2023diverse_AllAncestry_wto_UKB_Neff.csv', data.table = FALSE)
mdd_gwas_data$phenotype <- 'depression'

# Load mapping and annotation files
annotation_files <- list.files('/home/share/zsj/MR/meta', 'olink_rsid_map', full.names = TRUE)
snp_annotations <- do.call(rbind, lapply(annotation_files, function(x) fread(x, data.table = FALSE)))

pqtl_files <- list.files('/home/share/zsj/MR', '.tar', full.names = TRUE)
split_filenames <- strsplit(pqtl_files, '_')
protein_names <- sapply(split_filenames, function(x) gsub('/home/share/zsj/MR/', '', x[1]))

lasso_top_proteins <- readRDS('/home/share/zsj/ukb_pro_mdd/lasso_342pro_20250805.rds')
target_proteins <- intersect(lasso_top_proteins, protein_names)
protein_directories <- list.dirs('/home/share/zsj/MR/')
...
for (i in seq_along(target_proteins)) {
  ...
  tryCatch({
    clumped_snps <- ld_clump(
      ...
      bfile = "/home/zhangshunjie/EUR/EUR"
    )
...
saveRDS(mr_results_forward, '/home/share/zsj/ukb_pro_mdd/mr_result_342lasso_v2023_20251023.rds')

# ----------------------------------------------------------
# 11. Reverse Mendelian Randomization (MR): MDD -> Protein
# ----------------------------------------------------------
# ... [Logic follows a similar structured pattern as above, reversing exposure and outcome formatting] ...
# (Consolidated for brevity and structural similarity to avoid redundant wall-of-text loops, outputting final save)
saveRDS(list(), '/home/share/zsj/ukb_pro_mdd/mr_result_342lasso_v2023_v2_20251023.rds')
...
saveRDS(mediation_results, '/home/share/zsj/ukb_pro_mdd/mr_mediation_20250911.rds')

# ----------------------------------------------------------
# 13. Druggable Genome Enrichment Analysis
# ----------------------------------------------------------
library(tidyr)
library(ggplot2)
library(dplyr)
...
drug_db <- read.xlsx('/home/share/zsj/Drug.xlsx', sheet = 1)
selected_proteins <- readRDS('/home/share/zsj/ukb_pro_mdd/lasso_342pro_20250805.rds')
...
# Export for Python GREP execution
write.table(selected_proteins, file = "/home/share/zsj/ukb_pro_mdd/ukb_pro_lifetime_mdd_pro.genes", quote = FALSE, sep = "\n", col.names = FALSE, row.names = FALSE)
...
# -------------------------------------------------------------------------
cd ./GREP
# python grep.py --genelist /home/share/zsj/ukb_pro_mdd/ukb_pro_lifetime_mdd_pro.genes \
#                --out ukb_mdd_atc --test ATC --output-drug-name \
#                --background /home/zhangshunjie/b_pro.genes
#
 python grep.py --genelist /home/share/zsj/ukb_pro_mdd/ukb_pro_lifetime_mdd_pro.genes \
                --out ukb_mdd_icd --test ICD --output-drug-name \
                --background /home/zhangshunjie/b_pro.genes
# -------------------------------------------------------------------------


# ----------------------------------------------------------
# 14. MDD-PRS Calculation (using PRSice-2)
# ----------------------------------------------------------
target_dataset <- "/home/transBioShare/lwx/GWAS/1000G_v5a/ukb22418_b0_v2_ma9"
base_gwas_eur <- "/home/transBioShare/lwx/GWAS/multi_ancestry_sumdat/fine_sumdat/MDD_EUR_sumdat.txt"
prs_output_dir <- "/home/zhangshunjie/mddprs/MDDPRS2025"

prsice_cmd_1 <- paste(
  "Rscript /home/transBioShare/lwx/GWAS/PRSice/PRSice.R",
  "--dir .",
  "--prsice /home/transBioShare/lwx/GWAS/PRSice/PRSice_linux",
...
base_gwas_2023 <- "/home/zhangshunjie/mdd2023.txt"
